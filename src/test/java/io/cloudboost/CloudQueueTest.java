...
//		CloudApp.init("egimabengitest", "yiBh75txY35CB+LSb/1XLQ==");
	...
	@Test(timeout=10000)//should push data into the queue
		public void push() {
			initialize();
	...
	@Test(timeout=10000)//should push data array into the queue
	public void pushArray(){
		initialize();
		CloudQueue que=new CloudQueue("follow",null);
		que.push(new String[]{"sample","egima"},  new CloudQueueMessageCallback() {
			...
			public void done(QueueMessage[] msg, CloudException e) {
				...
				if(msg!=null){
					Assert.assertTrue(msg.length==2&&msg[0].getMessage().equals("sample")&&msg[1].getMessage().equals("egima"));
				}
				
			}
		});
	}
	@Test(timeout=10000)//should push message array into the queue
	public void pushMessageArray(){
		...
		que.push(msgs,  new CloudQueueMessageCallback() {
			...
			public void done(QueueMessage[] msg, CloudException e) {
				...
				if(msg!=null){
					Assert.assertTrue(msg.length==2&&msg[0].getMessage().equals("sample1")&&msg[1].getMessage().equals("sample2"));
				}
				
			}
		});
	}
	@Test(timeout=20000)//can push duplicate messages into same queue
					...
					que.push(msgs, new CloudQueueMessageCallback() {
						...
						public void done(QueueMessage[] msg, CloudException e) {
							...
							for(QueueMessage ms:msg){
								if(ms.getMessage().equals("sample")||ms.getMessage().equals("egima"))
									continue;
								else Assert.fail();
							}
	...
	@Test(timeout=20000)//should not push null data into queue
	public void noPushNull() {
		...
		que.push(null,  new CloudQueueMessageCallback() {
			...
			public void done(QueueMessage[] msgs, CloudException e) {
				if(e!=null){
					Assert.assertEquals(e.getMessage(), "Cannot push null into queue");
				}
				if(msgs!=null){
	...
	@Test(timeout=20000)//Should not allow null empty queue name
	public void dontAllowEmptyQueueName() {
		...
		que.push("sample",  new CloudQueueMessageCallback() {
			...
			public void done(QueueMessage[] msgs, CloudException e) {
				if(e!=null){
					Assert.assertEquals(e.getMessage(), "Object Validation Failure");
				}
				if(msgs!=null){
					...
					if(msg.getMessage().equals("sample")){
						que.pull(1, new CloudQueueMessageCallback() {
							...
							public void done(QueueMessage[] msgs, CloudException e) {
								...
								if(msgs!=null){
									Assert.assertTrue(msgs.length==1&&msgs[0].getMessage().equals("sample"));
								}
					...
					que.getMessageById(id,  new CloudQueueMessageCallback() {
						...
						public void done(QueueMessage[] msgs, CloudException e) {
							...
							if(msgs!=null){
								if(msgs.length==1){
								Assert.assertTrue(msgs.length==1&&msgs[0].getMessage().equals("sample"));}
								else Assert.fail("Should get only 1 message");
							}
									...
									if (msgs[0].getMessage().equals("sample2")) {
										que.peek(2,  new CloudQueueMessageCallback() {
											...
											public void done(QueueMessage[] msgs, CloudException e) {
												...
												if(msgs!=null){
													Assert.assertTrue(msgs.length==2&&msgs[0].getMessage().equals("sample1")&&msgs[1].getMessage().equals("sample2"));
												}
				...
				if(msgs!=null){
					que.peek(2,  new CloudQueueMessageCallback() {
						...
						public void done(QueueMessage[] msgs, CloudException e) {
							...
							if(msgs!=null){
								Assert.assertTrue(msgs.length==2&&msgs[0].getMessage().equals("sample"));
							}
					...
					if (msgs[0].getMessage().equals("sample1")) {
						que.push("sample2", new CloudQueueMessageCallback() {
							...
							public void done(QueueMessage[] msgs,
								...
								if (msgs != null) {
									if (msgs[0].getMessage().equals("sample2")) {
										que.pull(2,  new CloudQueueMessageCallback() {
	...
	public void shouldGetNullForWrongMessageById() {
		...
		que.push("sample",  new CloudQueueMessageCallback() {
			...
			public void done(QueueMessage[] msgs, CloudException e) {
				...
				if(msgs!=null){
					que.getMessageById("sample",  new CloudQueueMessageCallback() {
				...
				if(msgs!=null){
					...
					que.deleteMessage(id,  new CloudQueueMessageCallback() {
						...
						public void done(QueueMessage[] msgs, CloudException e) {
							...
							Assert.assertTrue(msgs!=null&&msgs[0].getId().equals(id));
		...
		que.addSubscriber("http://egima.blogspot.com",
				new CloudQueueCallback() {
					...
					public void done(CloudQueue q, CloudException e) {
						if(q!=null){
							try {
								JSONArray array=q.document.getJSONArray("subscribers");
								String subscribers=array.toString().replace("\"\"", "").replace("[", "").replace("]", "");
	...
	public void shouldDeleteSubscriber() {
		...
		que.addSubscriber(arr,
				new CloudQueueCallback() {
					...
					public void done(CloudQueue q, CloudException e) {
						if(q!=null){
							que.deleteSubscriber("http://egima.blogspot.com", new CloudQueueCallback() {
	...
	public void shouldDeleteMultipleSubscribers() {
		initialize();
		final CloudQueue que = new CloudQueue(PrivateMethod._makeString(), null);
		String[] arr={"http://egima.blogspot.com","http://nandala.com","http://sample.com"};
					...
					public void done(CloudQueue q, CloudException e) {
							...
							if(q!=null){
								if(q.subscribers.size()==1){
									que.removeSubscriber("http://thermodynamiccomputing.com/", new CloudQueueCallback() {
										...
										public void done(CloudQueue q, CloudException e) {
											...
											if(q!=null){
												try {
													JSONArray arr=q.document.getJSONArray("subscribers");
													Assert.assertTrue(arr.length()==0);
												} catch (JSONException e1) {
					...
					que.deleteQueue(queueName,new CloudQueueCallback() {
						...
						public void done(CloudQueue q, CloudException e) {
							if(e!=null)
								Assert.fail(e.getMessage());
							if(q!=null){
								System.out.println("got queue: "+q.document.toString());
								que.pull(1, new CloudQueueMessageCallback() {
									
									@Override
									public void done(QueueMessage[] msgs, CloudException e) {
										System.out.println("checking message msgs nullity");
										Assert.assertTrue(msgs==null);
	...
	@Test(timeout=10000)//should clear queue
	public void shouldClearQueue(){
		initialize();
		final CloudQueue que=new CloudQueue(UUID.uuid(8),null);
		que.push(new String[]{"sample","egima","ayiko","ben"},  new CloudQueueMessageCallback() {
					...
					que.clear(new CloudQueueCallback() {
						...
						public void done(CloudQueue q, CloudException e) {
							if(e!=null)
								Assert.fail(e.getMessage());
							if(q!=null){
								q.pull(1, new CloudQueueMessageCallback() {
				...
				if(msg!=null){
					que.get(new CloudQueueCallback() {
						...
						public void done(CloudQueue q, CloudException e) {
							...
							if(q!=null){
								try {
									int msgCount=q.document.getInt("totalMessages");
	...
	@Test(timeout=10000)//should get queue by name
	...
	@Test(timeout=100000)//should get queue by name
